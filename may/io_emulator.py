...
class IOEmulator (IOBase):
  ...
  def open_tempfile (self, mode="r", *optionals, **keyoptionals):
    if self.may.isdir(self.remotepath): # directory is not a file in this library xD
      raise PermissionError("could not open the directory %r like as file." % (self.remotepath,))
    tempname = self.get_tempname()
    if "r" in mode:
      if not self.may.exists(self.remotepath): # file must be exists on read mode xD
        raise FileNotFoundError("file %r was not found." % (self.remotepath,))
