...
class May:
  ...
  def chdir (self, directory):
    ...
    if not self.isdir(directory):
      raise FileNotFoundError("directory of %r was not found." % (directory,))
  ...
  def remove (self, file):
    ...
    if self.isdir(realpath):
      raise IsADirectoryError("%r is not a file. it is directory." % (realpath,))
  ...
  def rename (self, filefrom, fileto):
    ...
    if filefrom != fileto:
      ...
      raise SameFileError("%r and %r are same path." % (filefrom, fileto))
  ...
  def lastmod (self, path):
    ...
    if matched:
      year, month, day, hours, minutes, seconds = map(int, matched.groups())
  ...
  def makedirs (self, directory, exist_ok=False):
    ...
    for dir in chain(reversed(Path(realdirectory).parents), (Path(realdirectory),)):
      if dir != Path("/"): # ignore root directory!
        if self.isdir(dir):
          if not exist_ok:
            raise FileExistsError("%r was already exists." % (dir,))
        elif self.exists(dir):
          raise NotADirectoryError("%r was already exists, and it is not a directory." % (dir,))
        else:
          self.mkdir(dir)
  ...
  def upload (self, srcpath, distpath, blocksize=8192):
    ...
    with open(srcpath, "rb") as inputstream:
      ...
      self.ftp.storbinary("STOR %s" % (realpath.as_posix(),), inputstream, blocksize=blocksize)
  ...
  def download (self, srcpath, distpath, blocksize=8192):
    ...
    with open(distpath, "wb") as outputstream:
      ...
      self.ftp.retrbinary("RETR %s" % (realpath.as_posix(),), outputstream.write, blocksize=blocksize)
